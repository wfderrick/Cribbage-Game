...
def runs(cards, length, prev, run_len):
    if prev == -1:
        return runs(cards[1:], 1, cards[0], run_len) + runs([cards[0]] + cards[2:], 1, cards[1], run_len) + runs(cards[:2] + cards[3:], 1, cards[2], run_len) + runs(cards[:3] + cards[4:], 1, cards[3], run_len)
    elif len(cards) == 0:
        if length >= run_len:
            return length
        else:
...
def listruns(cards, length, prev, runs_list):
    if prev == -1:
        return listruns(cards[1:], 1, cards[0], runs_list) + listruns(cards[2:]+ [cards[0]], 1, cards[1], runs_list) + listruns(cards[:2] + cards[3:], 1, cards[2], runs_list) + listruns(cards[:3] + cards[4:], 1, cards[3], runs_list)
    elif len(cards) == 0:
...
example_hand = [structs.Card(structs.Suit.CLUBS, structs.Face.JACK, 10), structs.Card(structs.Suit.DIAMONDS, structs.Face.FIVE, 5), structs.Card(structs.Suit.SPADES, structs.Face.FIVE, 5),structs.Card(structs.Suit.HEARTS, structs.Face.FIVE, 5)]
